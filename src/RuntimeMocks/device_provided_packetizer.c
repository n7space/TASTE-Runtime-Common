...
packetizer_init_function getDeviceProvidedPacketizerPacketizeFunction(const enum SystemBus bus_id)
{
    if(bus_id > SYSTEM_BUSES_NUMBER)
    {
        return NULL;
...
packetizer_init_function getDeviceProvidedPacketizerDepacketizeFunction(const enum SystemBus bus_id)
{
    if(bus_id > SYSTEM_BUSES_NUMBER)
    {
        return NULL;
